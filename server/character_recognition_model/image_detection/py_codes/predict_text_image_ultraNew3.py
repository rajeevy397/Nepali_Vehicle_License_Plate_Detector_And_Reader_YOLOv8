...
if len(sys.argv) != 2:
    print("Usage: python predict_text_image_ultraNew3.py <image_path>")
    sys.exit(1)
    ...
    if score > threshold:
        ...
        if h != target_size[0] or w != target_size[1]:
            ...
            output_resized_object_path = os.path.join(
                output_folder, f'{idx + 1}_resized.jpg')
...
json_output_path = os.path.join(
    PATH, 'labels_output', 'detected_objects.json')
...
py_codes_path = os.path.join(
    '.','character_recognition_model', 'image_detection', 'py_codes')
...
subprocess.run(['python',os.path.join(py_codes_path,'write_character_onImage.py')])
