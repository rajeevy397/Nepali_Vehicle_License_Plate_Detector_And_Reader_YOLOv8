...
if len(sys.argv) != 2:
    print("Usage: python predict_text_video_ultraNew4.py <video_path>")
    sys.exit(1)

video_path = sys.argv[1]

py_codes_path = os.path.join(
    '.', 'character_recognition_model', 'video_detection', 'py_codes_2')  
...
image_files = [f for f in os.listdir(
    input_images_folder) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
...
for image_file in image_files:
    ...
    for idx, result in enumerate(results.boxes.data.tolist()):
        ...
        if score > threshold:
            ...
            if h != target_size[0] or w != target_size[1]:
                ...
                output_resized_object_path = os.path.join(
    ...
    json_output_path = os.path.join(
        PATH, 'labels_output', f'detected_objects.json')
    with open(json_output_path, 'w') as json_file:
        json.dump(detected_objects, json_file, indent=4)

    print(
        f"Detected objects saved as images and JSON file created at: {json_output_path}")
...
subprocess.run(['python', os.path.join(py_codes_path,'frames_toVideo2.py')])
